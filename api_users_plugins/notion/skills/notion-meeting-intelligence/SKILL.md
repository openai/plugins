...
## Workflow
### 0) If Notion tools are unavailable, pause and ask the user to connect the Notion MCP server:
1. Enable this plugin's Notion MCP server for the session.
2. Complete the Notion MCP OAuth flow if Codex prompts for it, or run `codex mcp login notion`.
3. Restart Codex or the current session if the tools still do not appear.

After the MCP server is connected, finish your answer and tell the user to retry so they can continue with Step 1.
