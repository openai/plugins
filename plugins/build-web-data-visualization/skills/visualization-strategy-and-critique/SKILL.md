...
10. For editorial infographics, report/deck figures, visual articles, composite layouts, animation, generated imagery, scrollytelling, parallax, or visualization placement inside an existing page, use `../../references/foundations/meaning-preserving-visual-design-workflow.md` and `../../references/foundations/mobile-first-responsive-visualization.md` when composition materially affects understanding. If the user asks for visual design, page layout, a mockup, a generated concept, a key frame, or to be shown what the visualization could look like, Codex image generation is mandatory before implementation or a text-only design handoff. Generate both large-screen and mobile portrait concepts, plus mobile landscape when the evidence or interaction needs a wide handheld design. The workflow must stop for user approval of the generated concept set before implementation continues, and the approved concepts must become a binding semantic design contract. After each generated concept set, provide concise bullets for the plan and any interactions, ask for approval or specific changes, and iterate with the user until the design is agreed.
...
- Use `../../references/foundations/meaning-preserving-visual-design-workflow.md` when Codex image-generated concepts would clarify layout, page integration, figure composition, key frames, or generated-asset roles. If the user requests visual design direction or asks to see what is being considered, generate the large-screen and mobile concept set now rather than answering with text alone. Show generated concepts to the user, summarize the proposed plan and interactions in concise bullets, and wait for explicit approval or requested iteration before development, file edits, or implementation code proceeds.
- Use `../../references/foundations/mobile-first-responsive-visualization.md` for the large-screen/mobile concept set, mobile portrait reading path, optional landscape design, touch/keyboard behavior, spotty-connection plan, and capability audit.
...
- Use generated images only when they add meaning or orientation. Keep numerical labels and source notes in editable data-bound layers wherever possible. Treat user-approved generated concepts as binding semantic contracts with locked layout, hierarchy, interaction, and data-layer commitments, not loose style references.
