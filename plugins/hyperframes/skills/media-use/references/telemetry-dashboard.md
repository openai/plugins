...
For each tile, confirm the event/property schema, create the corresponding
trend/breakdown insight, then collect those insights into a dashboard. Keep
names prefixed `media-use:` so the dashboard is greppable. Cross-surface note:
because identity is shared with CLI/studio, you can also break these down by the
same person across `cli_command*` and `studio:*` events.
