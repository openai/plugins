...
- **Step 5 (frame worker):** install the named block and fill it with the real diff/snippet (below). The block is the frame's centerpiece, composited onto the preset's navy **Code Surface**.
...
The theme is baked into each block (not chosen at runtime); to use a given look, install that block and edit its `codeLines`. For the preset's editorial register, prefer the focused animation blocks on the navy Code Surface; reach for a `code-snippet-*` UI only when "show it in a real editor/terminal" is the point.
...
- an **invented animated diagram** — SVG / HTML / GSAP the frame worker builds from the preset's atoms (hairline-ink nodes / edges / lanes on cream, one coral marker on the active element), the build playing out the behavior across the shot; **or**
...
| A benchmark / metric / count-up                          | **Not a `code-*` block** — use the preset's `number-lockup` (Number/Impact treatment) or the `data-chart` registry block                                             |
