...
2. Design the complete requested surface before coding. For a full page, app, dashboard, game, or product interface, a header or hero concept is not enough. Generate one full concept when practical; otherwise generate coordinated section, state, responsive, or asset concepts first.
...
- If one image would cram the surface, generate multiple coordinated concepts before coding. Keep one `accepted layout concept` as the structural spec; treat later images as supporting section, state, responsive, or asset specs.
