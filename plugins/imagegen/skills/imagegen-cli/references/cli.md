...
Resolve the CLI relative to the loaded `imagegen-cli` skill, then set a stable absolute path:

```
export IMAGE_GEN="<absolute-imagegen-cli-skill-path>/scripts/image_gen.py"
```
...
## See also
- API parameter quick reference for fallback CLI mode: `image-api.md`
- Prompt examples shared across both modes: `../../imagegen/references/sample-prompts.md`
- Network/sandbox notes for fallback CLI mode: `codex-network.md`
- Built-in-first transparent image workflow: `../../imagegen/SKILL.md` and its `scripts/remove_chroma_key.py`
