...
- After generation, run `scripts/remove_chroma_key.py` from the parent `imagegen` skill directory with `--input <source> --out <final.png> --auto-key border --soft-matte --transparent-threshold 12 --opaque-threshold 220 --despill`, then validate the alpha result before shipping it.
...
- If the user explicitly chooses CLI fallback, load the sibling `imagegen-cli` skill and its `references/cli.md` and `references/image-api.md` files.
