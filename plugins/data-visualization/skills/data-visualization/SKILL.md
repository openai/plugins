...
description: Route web data visualization work. Use when the user needs help choosing charts, shaping page layout or interaction, making visualization state URL-driven and shareable, persisting configured views, designing operational visualization workspaces, visually designing a visualization, generating Codex image-based design concepts for both large-screen and mobile views, selecting a visualization stack, choosing graph or node-link layout strategies, adding domain-native contextual backgrounds or graphical substrates, critiquing visuals, designing scrollytelling or parallax visual stories, implementing or testing browser visualizations, creating UML or software architecture diagrams, building dashboards, or exporting visuals into reports and documents. For advanced visual/page/layout design requests where composition affects understanding, Codex must generate and show a large-screen concept and a mobile portrait concept before implementation or text-only design handoff, plus a mobile landscape concept when the interaction or evidence needs it.
---
...
8. For editorial infographics, reports, decks, visual articles, scrollytelling, parallax, generated imagery, animation, composite layouts, or visualization placement inside existing pages, use `../../references/foundations/meaning-preserving-visual-design-workflow.md` and `../../references/foundations/mobile-first-responsive-visualization.md` when visual composition materially affects understanding. If the user asks to visually design a page, show a layout direction, create a concept, redesign the composition, or "help me design this" for an advanced visualization, image concept generation is mandatory: invoke Codex image generation in the current response before giving a text-only design handoff or starting implementation. Generate a concept set, not only one desktop image: large-screen and mobile portrait are required by default, and mobile landscape is required when the visualization uses a wide substrate, AR/camera/motion, two-handed gestures, or keyboard-heavy interaction. The workflow must pause after concept generation so the user can approve the design direction or request iteration before implementation continues. After showing each generated concept set, describe the proposed plan and interactions in concise bullet points, ask for explicit approval or change requests, and iterate with revised concepts until the user agrees on the design. User-approved concepts become binding semantic design contracts, not loose designs: preserve the claim, evidence hierarchy, labels, caveats, source context, animation purpose, layout intent, visual hierarchy, spatial relationships, interaction staging, mobile/landscape continuation, and export continuation.
...
- For advanced explanatory or composite surfaces where layout, imagery, animation, or page integration affects interpretation, use `../../references/foundations/meaning-preserving-visual-design-workflow.md`. Use Codex image generation for design concepts when it will clarify composition; this is required, not optional, when the user explicitly asks for visual design, page layout, a concept image, a visual direction, a mockup, a key frame, or to be shown what is being considered. Show the generated concept set to the user, including large-screen and mobile portrait concepts by default plus mobile landscape when needed. Summarize the plan and interactions in concise bullet points, and do not make project changes or generate implementation code until the user approves or requests a new iteration. Keep iterating with the user until the design is agreed, then treat the approved concepts as a binding contract tied to the evidence lock, with locked elements, flexible elements, approved deviations, mobile/landscape continuation, and concept-to-result QA.
...
- For concepted visual-design work before approval, show the generated concept set, including large-screen and mobile portrait images plus mobile landscape if needed. Provide concise bullets for the plan and interactions, then ask for approval or specific changes before making project changes or generating code. After approval, state the concept scope, user design approval status, approved concept paths or screenshot references, binding semantic design contract, locked elements, flexible elements, data-bound layers, mobile/landscape continuation, approved deviations, and concept-to-result fidelity QA checks.
