...
10. Do not run the notebook after editing unless the user explicitly asks for execution.
...
After a successful edit, report the created project, notebook, or block name/ID, updated or moved block IDs when relevant, plus placement or final block order when useful. Include Deepnote links when they can be safely constructed with `deepnote-links`.
