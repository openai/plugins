...
`get_file_content` requires a markdown or extracted-text representation. Use file signals to avoid sending obviously unsupported files into a text-content read:
...
3. If the file is obviously visual, binary, or preview-oriented, prefer preview or metadata paths before a text-content read.
...
If `get_file_content` returns `Markdown or text representation is not available for this file`, do not retry the same call. Use a preview path for previewable content, inspect metadata when it can answer the question, or choose a scoped fallback.
