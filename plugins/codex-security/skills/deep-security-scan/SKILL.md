...
- Preserve Codex Security's normal HTML-first final-report contract and review-directive behavior: render the primary readable `report.html` from its `report.md` source and return both paths.
...
5. assemble the final Codex Security report outputs once using the ordinary final-report guidance from `$codex-security:security-scan`: write `report.md`, render the primary readable `report.html`, and return both final report paths
...
- Write `report.md` as the markdown source, render `report.html` as the primary readable report using the ordinary final-report guidance, and include both final report paths in the response.
