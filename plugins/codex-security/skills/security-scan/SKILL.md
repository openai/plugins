---
name: security-scan
description: "Use when the user asks for a repository-wide or scoped-path security scan. Run distinct phases: threat modeling, finding discovery, validation, attack-path analysis, and final HTML plus markdown output."
metadata:
...
7. Assemble the final markdown report last using the final report path from `../../references/scan-artifacts.md`, then render the final HTML report using `references/final-report.md` and the outputs of the earlier phases: finding discovery plus each candidate finding's validation and attack-path reports.
...
If the user doesn't explicitly authorize subagents, stop and ask for the permission because the use of subagents is vital to the performance of exhaustive repository or scoped-path scans.
