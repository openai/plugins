...
description: "Use when the user asks for a security review of a pull request, commit, branch diff, working-tree patch, or other Git-backed change set. Run distinct phases: threat modeling, finding discovery, validation, attack-path analysis, and final markdown output."
metadata:
...
Used when a user wants to review a Git-backed change set for security regressions. Keep the scan phases separate and produce a final markdown report.
...
7. Assemble the final markdown report last using the final report path from `../../references/scan-artifacts.md` and the outputs of the earlier phases: finding discovery plus each candidate finding's validation and attack-path reports.
...
Assemble the final markdown report and Codex app review directives using `../security-scan/references/final-report.md`.
