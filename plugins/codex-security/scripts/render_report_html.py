...
def render_page(title: str, markdown: str) -> str:
...
<head>
  ...
  <style>
    :root {{ color-scheme: light dark; --bg: #fff; --bg-elev: #fafafa; --bg-muted: #f5f5f7; --fg: #111827; --muted: #6b7280; --border: #e5e7eb; --accent: #2563eb; --code: #f3f4f6; --shadow: 0 4px 10px rgba(0, 0, 0, 0.06); }}
    @media (prefers-color-scheme: dark) {{ :root {{ --bg: #0b0f14; --bg-elev: #0f141b; --bg-muted: #121923; --fg: #e5e7eb; --muted: #9ca3af; --border: #243040; --accent: #60a5fa; --code: #111827; --shadow: 0 4px 10px rgba(0, 0, 0, 0.35); }} }}
    * {{ box-sizing: border-box; }}
    ...
    .sidebar {{ min-height: 100vh; border-right: 1px solid var(--border); background: var(--bg-muted); }}
    .toc-inner {{ position: sticky; top: 0; height: 100vh; overflow: auto; padding: 10px 8px; }}
    .nav-title {{ display: flex; align-items: center; justify-content: space-between; gap: 8px; color: var(--muted); font-size: 11px; font-weight: 700; letter-spacing: 0.08em; margin: 0 0 8px; text-transform: uppercase; }}
    .nav-toggle {{ border: 1px solid var(--border); background: var(--bg); color: var(--fg); border-radius: 8px; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; padding: 5px; }}
    .nav-toggle:hover {{ box-shadow: var(--shadow); }}
    .nav-meta {{ color: var(--muted); font-size: 12px; margin: 0 0 12px; padding: 0 4px; }}
    body.sidebar-collapsed .toc-inner {{ display: flex; align-items: center; padding: 8px 6px; }}
    body.sidebar-collapsed .nav-title span, body.sidebar-collapsed .nav-meta, body.sidebar-collapsed .toc {{ display: none; }}
    ...
    main h3[id^="7-"] + table,
    main h3[id^="8-"] + table,
    main h3[id^="9-"] + table {{ margin: 10px 0 24px; border: 1px solid var(--border); border-left: 4px solid var(--accent); background: var(--bg-elev); box-shadow: var(--shadow); }}
    ...
    main h3[id^="9-"] + table td:first-child {{ width: 170px; color: var(--muted); font-weight: 700; white-space: nowrap; }}
    ...
    .toc a {{ display: block; border-radius: 6px; color: var(--fg); font-size: 13px; line-height: 1.3; padding: 4px 6px; position: relative; transition: background 160ms ease, color 160ms ease, padding-left 160ms ease; }}
    ...
    .toc-item.active > a {{ color: var(--accent); background: color-mix(in srgb, var(--accent) 12%, transparent); font-weight: 700; padding-left: 10px; }}
    .toc-item.active > a::before {{ content: ''; position: absolute; left: 0; top: 4px; bottom: 4px; width: 3px; border-radius: 2px; background: var(--accent); }}
    .muted {{ color: var(--muted); }}
    @media (max-width: 900px) {{ .layout {{ display: block; }} .sidebar {{ min-height: 0; border-right: 0; border-bottom: 1px solid var(--border); }} .toc-inner {{ position: static; height: auto; max-height: 45vh; }} main {{ padding: 24px; }} }}
  </style>
</head>
<body>
  <div class="layout">
    <aside class="sidebar" aria-label="Table of contents">
      ...
      <div class="nav-meta">{html.escape(title)}</div>
      <ul class="toc">
        {toc_items}
      </ul>
      </div>
...
def main() -> None:
    ...
    html_page = render_page(args.title, markdown)
